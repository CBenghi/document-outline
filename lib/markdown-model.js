  ...
  getHeadings(parsed) {
    ...
    while ((event = walker.next())) {
      ...
      if (node.type === 'heading') {
        ...
        } else { // exiting header node
          inHeading = false;
          heading.label = headingText;
        }
	  // Make sure we only consider the literal if it's not null #29
	  // also, the status icons only need checking if we have an heading to tag.
	  //
      } else if (node.literal && heading) {
		// expand the heading if in it
		if (inHeading) {  
			headingText += node.literal;
		}
		// don't check for todo in the header body
		else {
			if (node.literal.includes("{>> TODO: DONE:")) {
				heading.isDone = true;
			} else if (node.literal.includes("{>> TODO: LAST:")) {
				heading.hasPostponed = true;
			} else if (node.literal.includes("{>> TODO: NEXT:")) {
				heading.isNext = true;
			} else if (node.literal.includes("{>> TODO: IMG:")) {
				heading.needsVisual = true;
			} else if (node.literal.includes("{>> TODO:")) {
				heading.hasTodo = true;
			}
			// regardless from previous
			if (!heading.isDone && node.literal.includes("DONE:")) {
				heading.isDone = true;
			}
			if (!heading.needsVisual && node.literal.includes("TODO: IMG:")) {
				heading.needsVisual = true;
			}
		}
      }
