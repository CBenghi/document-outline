...
class _HeadingNode extends HTMLElement {
  ...
  setHeading(item) {
	...
	// add icon for todo if appropriate
	if (item.hasTodo) {
		this.todoSpan = createElement('span', {class: "statusIcon todo"});
		this.appendChild(this.todoSpan);
	}
	if (item.hasPostponed) {
		this.postponedSpan = createElement('span', {class: "statusIcon postponed"});
		this.appendChild(this.postponedSpan);
	}
	if (item.isNext) {
		this.nextSpan = createElement('span', {class: "statusIcon next"});
		this.appendChild(this.nextSpan);
	}
	if (item.needsVisual) {
		this.visualSpan = createElement('span', {class: "statusIcon visual"});
		this.appendChild(this.visualSpan);
	}
	if (item.isDone) {
		this.doneSpan = createElement('span', {class: "statusIcon done"});
		this.appendChild(this.doneSpan);
	}
